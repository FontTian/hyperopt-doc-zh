"""
TOTALLY BROKEN EXAMPLE DO NOT USE FOR ANYTHING
"""
import copy
import cPickle
import logging
...
class NewDBN(Bandit):
    def __init__(self, dataset, sup_min_epochs, sup_max_epochs):
        rng = np_rng_from_seed(choice(range(10)))
        t_rng = theano_random_streams(2345)

        dataset, train_Xy, valid_Xy, test_Xy = preprocess_data(argd, ctrl)

        potential_layers = []
        for ii in range(3):
            layer = train_rbm_shared(train_Xy[0],
                n_hid=qloguniform(np.log(128), np.log(4096), q=16),
                idist=one_of('uniform', 'normal', 'glorot'),
                iscale=loguniform(np.log(.1), np.log(10)),
                cd_epochs=qloguniform(np.log(0.5), np.log(3000), q=1),
                cd_batchsize=100,
                cd_sample_v0s=one_of(0, 1),
                cd_lr=lognormal(numpy.log(.01), 2),
                cd_lr_anneal_start=qloguniform(np.log(10), np.log(10000)))
            layers.append(layer)

        # use 1, 2, 3 layers with equal probability under prior
        layers = one_of(
                potential_layers[:1],
                potential_layers[:2],
                potential_layers[:3])

        nnet = fit_nnet(layers,
                train=train_Xy,
                valid=valid_Xy,
                min_epochs=sup_min_epochs,
                max_epochs=sup_max_epochs,
                batchsize=20,
                sgd_step_size0=lognormal(numpy.log(.01), 3),
                lr_anneal_start=geom(100, 10000),
                l2_penalty=one_of(0, lognormal(numpy.log(1.0e-6), 2)),
                )

        pred = model_predict(nnet, test_Xy[0])
        erate = error_rate(test_Xy[1], pred)
        Bandit.__init__(self, erate)
